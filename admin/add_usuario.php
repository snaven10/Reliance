...
<form action='' method='POST' role='form' class='col-xs-12 col-sm-12 col-md-6 col-lg-6' enctype='multipart/form-data'>
    ...
    <div class="input-group">
        ...
        <input type="text" name='Nombre' class='form-control'
            value='<?php if(!empty($_GET['id'])){echo $row[1];}else{if(!empty($_POST['Nombre'])){echo $_POST['Nombre'];}}?>'
    ...
    <div class="input-group">
        ...
        <input type="text" name='Usuario' class='form-control'
            value='<?php if(!empty($_GET['id'])){echo $row[2];}else{if(!empty($_POST['Usuario'])){echo $_POST['Usuario'];}}?>'
            placeholder='Usuario' aria-describedby="basic-addon1" required>
    </div>
    <br>
    <div class="input-group">
        <span class="input-group-addon" id="basic-addon1"><b>Password:</b></span>
        <input type='password' name='Password' class='form-control'
            value='<?php if(empty($_GET['id'])){if(!empty($_POST['Password'])){echo $_POST['Password'];}}?>'
    ...
    <div class="input-group">
        ...
        <select name='Nivel' class="form-control" aria-describedby="basic-addon1" required>
            ...
            <option value="1"
                <?= (!empty($_GET['id']))? (($row[4] == 1) ? 'selected' : '') : ((!empty($_POST['Nivel'])) ? (($_POST['Nivel'] == 1) ? 'selected' : '') : ''); ?>>
                Vendedor</option>
            <option value="2"
                <?= (!empty($_GET['id']))? (($row[4] == 2) ? 'selected' : '') : ((!empty($_POST['Nivel'])) ? (($_POST['Nivel'] == 2) ? 'selected' : '') : ''); ?>>
                Administrador</option>
            <option value="3"
                <?= (!empty($_GET['id']))? (($row[4] == 3) ? 'selected' : '') : ((!empty($_POST['Nivel'])) ? (($_POST['Nivel'] == 3) ? 'selected' : '') : ''); ?>>
